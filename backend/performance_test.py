...
class PerformanceTest:
    ...
    async def test_endpoint_performance(
        ...
        for _ in range(REQUESTS_PER_USER):
            ...
            try:
                ...
                errors += 1
                print(f"Erro no endpoint {endpoint}: {e!s}")
    ...
    async def run_performance_tests(self):
        ...
        print(
            f"✅ Carga concorrente: {concurrent_result['successful_sessions']}/{CONCURRENT_USERS} sessões bem-sucedidas"
        )
    ...
    def generate_summary(self):
        ...
        successful_endpoints = sum(
            1 for r in self.results["endpoints"].values() if "error" not in r
        )
        ...
        if successful_endpoints > 0:
            ...
            print(
                f"Tempo médio de resposta: {statistics.mean(avg_response_times):.3f}s"
            )
