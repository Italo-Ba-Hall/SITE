...
async def test_conversation_summary():
    ...
    chat_manager.add_message(
        session_id,
        "assistant",
        "Entendo! Que tipo de processo você gostaria de automatizar?",
    )
    chat_manager.add_message(session_id, "user", "Processos manuais repetitivos")
    chat_manager.add_message(
        session_id, "assistant", "Perfeito! Posso ajudar com RPA. Qual é seu nome?"
    )
...
async def test_lead_with_email():
    ...
    chat_manager.add_message(
        session_id, "assistant", "Ótimo! Que tipo de dados você gostaria de visualizar?"
    )
...
async def test_llm_personality():
    ...
    try:
        ...
        print(f"❌ Erro ao testar LLM: {e!s}")
