...
async def test_first_message_response():
    ...
    try:
        ...
        if "automação" in response.message.lower() or "automatização" in response.message.lower():
        ...
        print(f"❌ Erro ao testar primeira mensagem: {str(e)}")
...
async def test_generic_first_message():
    ...
    try:
        ...
        print(f"❌ Erro ao testar mensagem genérica: {str(e)}")
